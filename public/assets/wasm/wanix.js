  ...
  constructor(options2) {
    if (options2) {
      ...
      if (options2.keyMap) {
        ...
        for (let [k, v] of Object.entries(options2.keyMap)) this.mapKey.set(v, k);
      }
    }
  ...
  constructor(options2) {
    ...
    this.encode = function(value, encodeOptions) {
      ...
      if (options2.pack) {
        ...
        packedValues3.maxValues = options2.maxPrivatePackedValues || (sharedPackedObjectMap2 ? 16 : Infinity);
    ...
    this.findCommonStringsToPack = () => {
      ...
      return (options3) => {
        ...
        let position5 = this.pack ? options3.maxPrivatePackedValues || 16 : 0;
        if (!sharedValues)
...
var WASI = class WASI2 {
  ...
  constructor(args, env, fds, options2 = {}) {
    this.args = [];
    this.env = [];
    this.fds = [];
    debug.enable(options2.debug);
    this.args = args;
    this.env = env;
    this.fds = fds;
    const self = this;
...
// node_modules/@progrium/duplex/esm/index.js
var esm_exports = {};
__export(esm_exports, {
  ...
  Peer: () => Peer,
  PortConn: () => Conn2,
  PortSession: () => Session2,
  Respond: () => Respond,
  RespondMux: () => RespondMux,
  Response: () => Response2,
  ResponseHeader: () => ResponseHeader,
  Session: () => Session,
  VirtualCaller: () => VirtualCaller,
  channelMaxPacket: () => channelMaxPacket,
...
// node_modules/@progrium/duplex/esm/fn/handler.js
...
// node_modules/@progrium/duplex/esm/transport/websocket.js
...
function connect(addr, onclose) {
  return new Promise((resolve) => {
    const socket = new import_ws2.default(addr);
...
// node_modules/@progrium/duplex/esm/peer/mod.js
var options = {
...
// node_modules/@progrium/duplex/esm/io.js
var EOF = null;
async function copy2(dst, src3) {
  const buf = new Uint8Array(32 * 1024);
  let written = 0;
  while (true) {
    const n = await src3.read(buf);
    if (n === EOF) {
      break;
    }
...
// node_modules/@progrium/duplex/esm/port/session.js
var Session2 = class {
  constructor(port) {
    Object.defineProperty(this, "port", {
      enumerable: true,
    ...
    this.port.onmessage = (event) => {
      ...
      if (msg.type === "open") {
        const ch = new Channel2(this, msg.port);
        this.channels.push(ch);
  ...
  async open() {
    ...
    const ch = new Channel2(this, msgCh.port1);
...
var WanixHandle2 = class {
  constructor(port) {
    const sess = new Session(new Conn2(port));
...
// helpers.js
function setupConsoleHelpers(id) {
...
// runtime.js
var WanixRuntime = class extends WanixHandle2 {
  constructor(config = {}) {
  ...
  async _loadBundle(bundle) {
    ...
    let foundWasm = false;
    await (0, import_js_untar.default)(this._bundle.slice()).progress((f) => {
...
if (globalThis.window) {
  ...
  window.WanixHandle = WanixHandle2;
}

// elements/terminal.js
var Terminal = class extends HTMLElement {
  ...
  async _connectToSource() {
    ...
    const src3 = this.getAttribute("src");
    if (!src3 || !this._system) return;
    this._disconnectFromSource();
    try {
      await this._system.ready();
      const readable = await this._system.openReadable(src3);
      this._reader = readable.getReader();
      this._readLoop();
      const writable = await this._system.openWritable(src3);
  ...
  _getOptionsFromAttributes() {
    ...
    if (this.hasAttribute("font-size")) {
      options2.fontSize = parseInt(this.getAttribute("font-size"), 10);
    }
    if (this.hasAttribute("font-family")) {
      options2.fontFamily = this.getAttribute("font-family");
    }
    if (this.hasAttribute("cursor-blink")) {
      options2.cursorBlink = this.getAttribute("cursor-blink") !== "false";
    }
    if (this.hasAttribute("cursor-style")) {
      options2.cursorStyle = this.getAttribute("cursor-style");
    }
    if (this.hasAttribute("scrollback")) {
      options2.scrollback = parseInt(this.getAttribute("scrollback"), 10);
    }
    return options2;
  }
  fit() {
...
// elements/system.js
var System = class extends HTMLElement {
  constructor() {
    super();
    this.runtime = null;
...
var DEFAULT_PROFILE = serializeWorkbenchProfile(DEFAULT_PROFILE_RAW);
function resolveWorkbenchBase(el) {
  ...
  try {
    const url = /^[a-zA-Z][a-zA-Z0-9+.-]*:/.test(value) ? new URL(value) : new URL(value, window.location.origin);
...
var Workbench = class extends HTMLElement {
  constructor() {
    super();
    this._loaded = false;
    this.port = void 0;
  ...
  _createWorkbench(portCb) {
    ...
    const config = {
      messagePorts: /* @__PURE__ */ new Map([["progrium.apptron-system", ch.port1]]),
    ...
    __require(["vs/workbench/workbench.web.main"], (wb) => {
      wb.create(this, {
        ...config,
        workspaceProvider: {
          trusted: true,
          workspace: { folderUri: wb.URI.revive(config.folderUri) },
          open(workspace, options2) {
