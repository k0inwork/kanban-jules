...
const ASSET_READY_TIMEOUT_MS = 60000;
const ASSET_READY_POLL_MS = 2000;
let configuredApiUrl = DEFAULT_API_URL;
export function setApiUrl(url) {
    configuredApiUrl = url.replace(/\/+$/, '');
}
function buildHFUrl(info) {
    if (info.url) {
        return info.url;
    }
    return `${HF_BASE_URL}/${info.repo}/resolve/main/${info.file}`;
}
function getLegacyRuntimeManifest(interpreter) {
    if (interpreter !== 'gpt2') {
        return null;
    }
    return {
...
export function getBaseModelUrl(runtime) {
    const info = runtime.js_sdk.base_model;
    if (!info) {
        throw new Error(`Runtime "${runtime.runtime_id}" is missing a browser base model definition.`);
    }
    return buildHFUrl(info);
}
function getProgramAssetUrl(programId, filename) {
    return `${HF_BASE_URL}/${HF_PROGRAMS_REPO}/resolve/main/${programId}/${filename}`;
}
function getMetaUrl(programId) {
    return getProgramAssetUrl(programId, 'meta.json');
}
function getPromptUrl(programId, promptFilename = 'prompt_template.txt') {
    return getProgramAssetUrl(programId, promptFilename);
}
export async function getRuntimeManifest(runtimeId) {
...
export async function resolveRuntimeManifest(meta) {
    ...
    throw new Error(`Browser inference does not support runtime "${meta.runtime_id || meta.interpreter}".`);
}
...
async function waitForAssetReady(url, label, optional = false) {
...
export async function loadProgramAssets(programId, _onProgress) {
    ...
    if (!runtime.js_sdk.supported) {
        throw new Error(`Browser inference does not support runtime "${runtime.runtime_id}" (${runtime.interpreter}).`);
    }
    const hydratedMeta = {
        ...meta,
        runtime,
    ...
    const adapterUrl = getProgramAssetUrl(programId, runtime.program_assets.adapter_filename || 'adapter.gguf');
    const promptUrl = getPromptUrl(programId, typeof runtime.prompt_template?.filename === 'string'
