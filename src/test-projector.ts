...
export async function seedTestData() {
  ...
  const docs = [
    { title: 'Architecture Overview', type: 'design', summary: '4-layer agent hierarchy with Yuan, Overseer, Architect, Programmer', tags: ['architecture', 'agents'], layer: ['L0', 'L1', 'L2'], projectId: 'test-project-id' },
    { title: 'API Reference', type: 'reference', summary: 'Available API tools: negotiator.askUser, negotiator.sendUser, kb.recordEntry, execute', tags: ['api', 'tools'], layer: ['L2', 'L3'], projectId: 'test-project-id' },
    { title: 'Security Policy', type: 'spec', summary: 'No credentials in logs, sanitize all user input, audit DB writes', tags: ['security', 'policy'], layer: ['L0', 'L1', 'L2', 'L3'], projectId: 'test-project-id' },
    { title: 'Testing Guidelines', type: 'spec', summary: 'Integration tests over unit mocks, real DB for all test suites', tags: ['testing', 'guidelines'], layer: ['L2', 'L3'], projectId: 'test-project-id' },
    { title: 'Deployment Runbook', type: 'reference', summary: 'Step-by-step deploy: build, test, push, verify health check', tags: ['deploy', 'ops'], layer: ['L0', 'L1'], projectId: 'test-project-id' },
    { title: 'Self-System Design', type: 'design', summary: 'How the kanban orchestrator itself is structured', tags: ['architecture', 'self'], layer: ['L0'], projectId: SELF_PROJECT_ID },
    { title: 'React Patterns', type: 'reference', summary: 'useLiveQuery hooks, IndexedDB boolean quirks, component state', tags: ['react', 'frontend'], layer: ['L3'], projectId: 'test-project-id' },
    { title: 'Git Workflow', type: 'spec', summary: 'Feature branches from main, squash merge, conventional commits', tags: ['git', 'workflow'], layer: ['L2', 'L3'], projectId: 'test-project-id' },
    { title: 'Agent Communication Protocol', type: 'design', summary: 'Message format between layers: type, sender, proposedTask', tags: ['agents', 'protocol', 'communication'], layer: ['L0', 'L1', 'L2'], projectId: 'test-project-id' },
    { title: 'Performance Budgets', type: 'spec', summary: 'Context window budgets per layer: L0=7200, L1=5400, L2=6000, L3=3600 chars', tags: ['performance', 'budget'], layer: ['L0', 'L1', 'L2'], projectId: 'test-project-id' },
  ];
  ...
  const entries = [
    ...
    { text: 'Self-insight: KBBrowser needs better abstraction-level filtering for large datasets', cat: 'observation', abs: 6, layer: ['L0'], tags: ['kb', 'ui'], src: 'dream', age: d(6), projectId: SELF_PROJECT_ID },
    { text: 'Self: projector budget math is correct — 10 docs at ~150 chars each fits L0 RAG budget', cat: 'observation', abs: 5, layer: ['L0'], tags: ['projector', 'budget'], src: 'dream', age: d(3), projectId: SELF_PROJECT_ID },
  ];
...
export async function runTests() {
  ...
  const l0 = await ProjectorHandler.project({ layer: 'L0', projectId: 'test-project-id', taskDescription: 'system review' });
  ...
  const l1 = await ProjectorHandler.project({ layer: 'L1', projectId: 'test-project-id', taskDescription: 'board review' });
  ...
  const l2 = await ProjectorHandler.project({ layer: 'L2', projectId: 'test-project-id', taskDescription: 'design feature' });
  ...
  const l3jules = await ProjectorHandler.project({ layer: 'L3', projectId: 'test-project-id', executor: 'executor-jules', taskDescription: 'clone repo' });
  ...
  const l3local = await ProjectorHandler.project({ layer: 'L3', projectId: 'test-project-id', executor: 'executor-local', taskDescription: 'build project' });
  ...
  const l3gh = await ProjectorHandler.project({ layer: 'L3', projectId: 'test-project-id', executor: 'executor-github', taskDescription: 'list issues' });
  ...
  const rag1 = await ProjectorHandler.project({ layer: 'L2', projectId: 'test-project-id', taskDescription: 'architecture agents design' });
  ...
  const rag2 = await ProjectorHandler.project({ layer: 'L3', projectId: 'test-project-id', executor: 'executor-local', taskDescription: 'react frontend components useLiveQuery' });
  ...
  const rag3 = await ProjectorHandler.project({ layer: 'L3', projectId: 'test-project-id', taskDescription: 'deploy runbook ops' });
  assert('no deployment runbook (L0/L1 only)', !rag3.includes('Deployment Runbook'));

  console.log('\n  TEST 2.4: L1 gets deployment runbook');
  const rag4 = await ProjectorHandler.project({ layer: 'L1', projectId: 'test-project-id', taskDescription: 'deploy runbook ops' });
  assert('deployment runbook present', rag4.includes('Deployment Runbook'));

  console.log('\n  TEST 2.5: Security policy visible to ALL layers');
  const ragL0 = await ProjectorHandler.project({ layer: 'L0', projectId: 'test-project-id', taskDescription: 'security policy credentials' });
  const ragL3 = await ProjectorHandler.project({ layer: 'L3', projectId: 'test-project-id', executor: 'executor-local', taskDescription: 'security policy credentials' });
  ...
  const ragSelf = await ProjectorHandler.project({ layer: 'L0', projectId: SELF_PROJECT_ID, taskDescription: 'self system design architecture' });
  ...
  const ragIrrel = await ProjectorHandler.project({ layer: 'L2', projectId: 'test-project-id', taskDescription: 'quantum computing neutrino physics' });
  ...
  const exp1 = await ProjectorHandler.project({ layer: 'L3', projectId: 'test-project-id', executor: 'executor-jules', taskDescription: 'clone repo timeout' });
  ...
  const exp2 = await ProjectorHandler.project({ layer: 'L1', projectId: 'test-project-id', taskDescription: 'jules checkout recurring error' });
  ...
  const exp3 = await ProjectorHandler.project({ layer: 'L3', projectId: 'test-project-id', executor: 'executor-jules', taskDescription: 'log parsing regex' });
  ...
  const exp4 = await ProjectorHandler.project({ layer: 'L3', projectId: 'test-project-id', executor: 'executor-local', taskDescription: 'file edit refactor' });
  ...
  const exp5 = await ProjectorHandler.project({ layer: 'L1', projectId: 'test-project-id', taskDescription: 'rag architecture decision keyword scoring' });
  assert('has RAG decision (abs=8)', exp5.includes('keyword scoring over embedding'));

  console.log('\n  TEST 3.6: Dream-sourced entries visible');
  const exp6 = await ProjectorHandler.project({ layer: 'L1', projectId: 'test-project-id', taskDescription: 'parallel tasks resource conflict dream' });
  ...
  const expTarget = await ProjectorHandler.project({ layer: 'L0', projectId: 'test-project-id', taskDescription: 'kb ui browser abstraction' });
  ...
  const expSelf = await ProjectorHandler.project({ layer: 'L0', projectId: SELF_PROJECT_ID, taskDescription: 'kb browser ui abstraction' });
  ...
  const empty = await ProjectorHandler.project({ layer: 'L2', projectId: 'test-project-id' });
  ...
  const l0board = await ProjectorHandler.project({ layer: 'L0', projectId: 'test-project-id', taskDescription: 'review' });
  assert('board section present', l0board.includes('## Board:'));

  console.log('\n  TEST 4.3: L3 does NOT get board state');
  const l3noBoard = await ProjectorHandler.project({ layer: 'L3', projectId: 'test-project-id', executor: 'executor-local', taskDescription: 'code' });
  assert('no board section', !l3noBoard.includes('## Board:'));

  console.log('\n  TEST 4.4: Budget respected — projection not absurdly long');
  const l3big = await ProjectorHandler.project({ layer: 'L3', projectId: 'test-project-id', executor: 'executor-local', taskDescription: 'architecture security testing react git deploy performance agents protocol' });
  ...
  const multiKw = await ProjectorHandler.project({ layer: 'L2', projectId: 'test-project-id', taskDescription: 'testing guidelines spec integration' });
